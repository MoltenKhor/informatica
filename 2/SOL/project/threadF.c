   ...
   /* if((n=readn(connfd, &howmany, sizeof(long)))==-1){
    ...
    file_size = *(size_t*)iov[2].iov_base;
      ...
      //  SYSCALL_EXIT("read", notused, readn(connfd, &path_len, sizeof(long)), "read", "");
       ...
       // SYSCALL_EXIT("readn path len", notused, readn(connfd, path_name, path_len*sizeof(char)), "read", "");
...
void threadF(void *arg){
    ...
    do{
       ...
       switch(action){
            case 'w': w_action(connfd, memory, iov);
            case 'W': //W_action(connfd, &str);
            case 'r': //r_action(connfd, &str);
            case 'R': //R_action(connfd, &str);
            default: break;
        } 
        break;
